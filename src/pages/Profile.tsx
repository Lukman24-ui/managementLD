...
import { 
  DollarSign, Palette, RefreshCw, ChevronRight, Settings,
  LogOut, Shield, HelpCircle, Moon, Dumbbell, Target,
  BookHeart, Link2, Loader2 
} from "lucide-react";
import { useState } from "react";
import NotificationSettings from "@/components/couple/NotificationSettings";

const Profile = () => {
  // Asumsikan useAuth mengembalikan state loading
  const { user, profile, partnerProfile, couple, signOut, updateProfile, loading } = useAuth(); 
  const navigate = useNavigate();
  const [loggingOut, setLoggingOut] = useState(false);

  // --- Penanganan Loading & User State ---
  if (loading) {
    return (
...
  // Fallbacks yang disempurnakan
  const userName = profile?.full_name || 'Pengguna';
  // Gunakan email asli dari profil, atau buat fallback
  const userEmail = profile?.email || `${profile?.full_name?.toLowerCase().replace(/\s/g, '.')}@email.com` || 'email@example.com';
...
  // Tipe stats ditingkatkan untuk konsistensi
  type StatColor = "turquoise" | "mint" | "happiness";
  const stats: { icon: any; label: string; value: number; color: StatColor }[] = [
    { icon: BookHeart, label: "Mood", value: 85, color: "turquoise" },
    { icon: Target, label: "Kebiasaan", value: 78, color: "mint" },
    { icon: Dumbbell, label: "Kebugaran", value: 92, color: "happiness" },
  ];
...
            {/* user sudah dijamin ada berkat if (!user) di atas */}
            <AvatarUpload
              userId={user.id}
